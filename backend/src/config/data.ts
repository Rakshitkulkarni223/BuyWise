import { WeightProfile, WeightProfileKey } from '../types';
...
/** Static mock-pricing "personality" for a supplier. */
export interface SupplierProfile {
  name: string;
  color: string; // hex used by the UI badge
  priceFactor: number; // multiplier on the catalog base price
  baseRating: number; // 0..5
  deliveryDays: number; // typical delivery time
  discountBias: number; // typical discount percent
  warrantyMonths: number; // 0 = not applicable
  returnDays: number;
  stockProbability: number; // 0..1
}
...
export const SUPPLIER_PROFILES: Record<string, SupplierProfile> = {
  ...
  Blinkit: { name: 'Blinkit', color: '#F8CB46', priceFactor: 1.08, baseRating: 4.6, deliveryDays: 0, discountBias: 5, warrantyMonths: 0, returnDays: 2, stockProbability: 0.8 },
  Zepto: { name: 'Zepto', color: '#7E3FF2', priceFactor: 1.1, baseRating: 4.4, deliveryDays: 0, discountBias: 4, warrantyMonths: 0, returnDays: 2, stockProbability: 0.78 },
  BigBasket: { name: 'BigBasket', color: '#84C225', priceFactor: 0.96, baseRating: 4.4, deliveryDays: 1, discountBias: 14, warrantyMonths: 0, returnDays: 3, stockProbability: 0.9 },
  JioMart: { name: 'JioMart', color: '#008ECC', priceFactor: 0.92, baseRating: 4.7, deliveryDays: 1, discountBias: 20, warrantyMonths: 0, returnDays: 5, stockProbability: 0.92 },
  Instamart: { name: 'Instamart', color: '#FC8019', priceFactor: 1.07, baseRating: 4.3, deliveryDays: 0, discountBias: 6, warrantyMonths: 0, returnDays: 2, stockProbability: 0.78 },
...
export const CATEGORY_SUPPLIERS: Record<string, string[]> = {
...
export const DEFAULT_CATEGORY_BASE_PRICE: Record<string, number> = {
  electronics: 24000,
  grocery: 600,
  fashion: 2500,
  furniture: 15000,
