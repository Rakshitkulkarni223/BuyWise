...
export function exportToCSV(products: Product[], query: string): void {
  try {
    ...
    downloadFile(csv, `ProcureAI_${sanitize(query)}_comparison.csv`, 'text/csv');
  } catch {
    // silent
  }
}
...
<html>
<head>
  <title>ProcureAI Report — ${query}</title>
  <style>
...
<body>
  <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:24px;">
    <div>
      <h1 style="margin:0;font-size:22px;">ProcureAI</h1>
  ...
  <p style="margin-top:32px;font-size:11px;color:#94a3b8;text-align:center;">
    Generated by ProcureAI · AI-Powered Procurement Intelligence · ${now}
  </p>
</body>
</html>`;
